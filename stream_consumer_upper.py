...
def bollinger_bands(stock_data, series_values, periods, dev_bands):
    ...
    upper_band = movil_average + (dev_movil * dev_bands)
    print(f'Symbol: {symbol}, Current_value: {round(current_value, 3)}, \
    Bollinger_upper_band: {round(upper_band.iloc[-1], 3)}')

    if current_value > upper_band.iloc[-1]:
        print(f'*** ALERT *** The value is above the range: '
              f'{upper_band.iloc[-1]} current value: {current_value}')
